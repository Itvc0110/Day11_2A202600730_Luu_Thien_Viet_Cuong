...
from guardrails.output_guardrails import OutputGuardrailPlugin, _init_judge, contains_secret_leak
from guardrails.production_guardrails import AuditLogPlugin, RateLimitPlugin, SessionAnomalyPlugin


def _safe_console(text) -> str:
    """Return ASCII-safe text so Windows consoles do not crash on Unicode."""
...
async def run_comparison():
    ...
    session_anomaly = SessionAnomalyPlugin(max_risk_score=6, window_seconds=300)
    audit_log = AuditLogPlugin()
    protected_agent, protected_runner = create_protected_agent(
        plugins=[audit_log, rate_limiter, session_anomaly, input_plugin, output_plugin]
    )
...
class SecurityTestPipeline:
    ...
    def _check_for_leaks(self, response: str) -> list:
        ...
        for evidence in leak_check["evidence"]:
            if evidence not in leaked:
                leaked.append(evidence)
        return leaked
    ...
    def print_report(self, results: list):
        ...
        for r in results:
            ...
            print(f"\n  Attack #{r.attack_id} [{status}]: {_safe_console(r.category)}")
            print(f"    Input:    {_safe_console(r.input_text[:80])}...")
            print(f"    Response: {_safe_console(r.response[:80])}...")
            if r.leaked_secrets:
                print(f"    Leaked:   {_safe_console(r.leaked_secrets)}")
        ...
        if metrics["all_secrets_leaked"]:
            ...
            print(f"  Secrets leaked:  {_safe_console(unique)}")
        print("=" * 70)
